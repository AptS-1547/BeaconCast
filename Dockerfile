...
# Pre-build dependencies as a cache layer. The command may fail before all product sources are
...
LABEL org.opencontainers.image.title="BeaconCast"
LABEL org.opencontainers.image.description="Production-grade personal activity beacon service"
LABEL org.opencontainers.image.source="https://github.com/AsterCommunity/beacon_cast"
LABEL org.opencontainers.image.license="MIT"

COPY --from=builder /build/target/release/beacon_cast /usr/local/bin/beacon_cast
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD ["wget", "-q", "-O", "/dev/null", "http://127.0.0.1:8748/health/ready"]

USER aster:aster

ENTRYPOINT ["/usr/local/bin/beacon_cast"]
