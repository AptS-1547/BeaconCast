...
fn default_config_uses_generated_data_paths() {
    let config = beacon_cast::config::AppConfig::default();

    assert_eq!(config.server.temp_dir, ".tmp");
    assert_eq!(config.database.url, "sqlite://beacon_cast.db?mode=rwc");
...
fn default_config_file_is_created_under_data_dir() {
    ...
    unsafe {
        std::env::set_var(beacon_cast::config::CONFIG_ENV_VAR, &config_path);
    }
    let loaded = beacon_cast::config::load().expect("load config");
    ...
    assert_eq!(
        ...
        format!(
            "sqlite://{}?mode=rwc",
            runtime_relative_path(dir.join("data").join("beacon_cast.db"))
        )
    );
    assert!(config_path.exists());
    assert!(generated.contains("# beacon_cast configuration file"));
    assert!(generated.contains(r#"temp_dir = ".tmp""#));
    assert!(generated.contains(r#"url = "sqlite://beacon_cast.db?mode=rwc""#));
...
fn environment_overrides_static_config() {
    ...
    unsafe {
        std::env::remove_var(beacon_cast::config::CONFIG_ENV_VAR);
...
impl EnvGuard {
    fn capture() -> Self {
        ...
        Self {
            _lock: lock,
            config_path: std::env::var_os(beacon_cast::config::CONFIG_ENV_VAR),
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        unsafe {
            restore_env_var(
                beacon_cast::config::CONFIG_ENV_VAR,
