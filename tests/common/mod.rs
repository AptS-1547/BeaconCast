...
#[allow(dead_code)]
pub async fn setup() -> beacon_cast::runtime::AppState {
    let mut config = beacon_cast::config::AppConfig::default();
    ...
    beacon_cast::runtime::assembly::prepare_state(config)
        .await
    ...
    ($state:expr) => {{
        ...
        test::init_service(
            App::new()
                ...
                .configure(beacon_cast::api::configure),
        )
        .await
    }};
}
