...
use beacon_cast::api::openapi::ApiDoc;
use utoipa::OpenApi;
...
fn generated_openapi_contains_health_route() {
    ...
    assert!(value["components"]["schemas"].get("ApiErrorCode").is_some());
    assert!(value["paths"].get("/api/v1/beacon/now").is_some());
    assert!(value["paths"].get("/api/v1/beacon/signals").is_some());
}
