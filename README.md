...
BeaconCast is a production-grade personal activity beacon service for ESAP's public website. It
collects trusted local activity signals, applies server-owned visibility policy, and publishes a
sanitized public status page plus reviewable activity history.

The service baseline comes from the AsterForge `aster-service` template and keeps the same Actix,
runtime, configuration, migration, embedded-frontend, and health-check shape.
...
```bash
docker build --build-arg CARGO_FEATURES=metrics,openapi -t beacon_cast .
```
